...
export function attachSockets(server) {
  ...
  wss.on('connection', openSession);
  heartbeat(wss);
  return wss;
}

function gateUpgrade(wss, req, socket, head) {
...
function openSession(ws, req) {
  ...
  const session = { room, code, me: null };
...
function onMessage(ws, session, msg) {
  if (msg.t === 'join') {
...
function onMessageFailed(ws, session, msg, e) {
  if (e instanceof Err) {
  ...
  log.throttle('error', 'message_failed', { code: session.code, pid: session.me?.id, t: msg.t, err: e }, `msg:${e?.message}`);
